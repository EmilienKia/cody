...
wxString StyleDef::toString()const
{
    ...
    if(bold.set())
    {
        if(*bold) arr.Add("bold");
        else arr.Add("notbold");
    }
    if(italic.set())
    {
        if(*italic) arr.Add("italic");
        else arr.Add("notitalic");
    }
    if(underline.set())
    {
        if(*underline) arr.Add("underline");
        else arr.Add("notunderline");
    }
    if(eolfilled.set())
    {
        if(*eolfilled) arr.Add("eolfilled");
        else arr.Add("noteolfilled");
    }
    if(font)
    {
        arr.Add(wxString("font:")+*font);
    }
    if(size)
    {
        arr.Add(wxString::Format("size:%d", *size));
    }
//	if(weight){arr.Add(wxString::Format("weight:%d", *weight)); }
    if(charcase)
    {
        arr.Add(wxString("case:")+*charcase);
    }
    if(fore)
    {
        arr.Add(wxString("fore:")+(*fore).GetAsString(wxC2S_HTML_SYNTAX));
    }
    if(back)
    {
        arr.Add(wxString("back:")+(*back).GetAsString(wxC2S_HTML_SYNTAX));
    }
